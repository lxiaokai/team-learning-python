...
if __name__ == '__main__':
    ...
    if soup == '':
        ...
        for h in href:
            if not h == '.':
                # 获取区
                numCode_county, strTr_county, href_county,now_request_url = down_county_data(now_path, strTr[href.index(h)], h)
            for hc in href_county:
                if not hc == '.':
                    try:
                        # 获取街道
                        numCode_town, strTr_town, href_town, now_request_url_town=down_town_data(now_path+'/'+strTr[href.index(h)], strTr_county[href_county.index(hc)], hc,now_request_url)

                        # for ht in href_town:
                        #     if not ht == '.':
                        #         try:
                        # 获取居委会
                        #             down_town_data(now_path + '/' + strTr[href.index(h)] + '/' + strTr_county[href_county.index(hc)],strTr_town[href_town.index(hc)], ht, now_request_url_town)
                        #         except:
                        #             print('解析居委会出错')
                    except:
                        print('解析街道出错:',strTr_county)
                        print('解析街道出错:',href_county)
                        print('解析街道出错:',hc)
